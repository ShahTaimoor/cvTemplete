...
export const educationDates = (e) => formatDate(e.startDate, e.endDate);

export const skillLabel = (s) => (s.level ? `${s.name} · ${s.level}` : s.name);

// Only http(s) URLs become real links — user-entered text like "javascript:…"
// must never end up in an href. Anything else is shown as plain text.
const safeHref = (url) => {
  const raw = (url || '').trim();
  if (!raw) return null;
  try {
...
export const UrlLink = ({ url, className = '', style }) => {
  const text = (url || '').trim();
  if (!text) return null;
...
export const SectionTitle = ({ children, style, variant = 'default' }) => {
  if (variant === 'minimal') {
    return (
      <h2 data-keep-with-next className="text-xs font-bold tracking-widest text-gray-400 mb-2 uppercase">{children}</h2>
    );
  }
  if (variant === 'elegant') {
    return (
      <h2 data-keep-with-next className="text-sm font-semibold mb-2 italic" style={{ color: style.primary, fontFamily: style.font }}>
        {children}
      </h2>
    );
  }
  ...
  if (variant === 'underline-light') {
    return (
      <h2 data-keep-with-next className="text-xs font-bold uppercase tracking-widest mb-2 pb-1 border-b border-gray-200 text-gray-700">
        {children}
      </h2>
    );
  }
      ...
      case 'summary':
        return resume.summary ? (
          <div key={key} className="mb-4">
            ...
            <p data-keep-together className="text-gray-600 whitespace-pre-wrap text-[11px] leading-relaxed">{resume.summary}</p>
          </div>
        ) : null;
      case 'experience':
        return resume.experience?.length ? (
          <div key={key} className="mb-4">
            ...
            {resume.experience.map((e, i) => (
              <div key={i} data-keep-together className="mb-3">
        ...
        return resume.education?.length ? (
          <div key={key} className="mb-4">
            ...
            {resume.education.map((e, i) => (
              <div key={i} data-keep-together className="mb-2">
                ...
                {educationDates(e) && <div className="text-[10px] text-gray-500">{educationDates(e)}</div>}
                {e.description && (
                  <p className="text-gray-600 text-[10px] mt-0.5 whitespace-pre-wrap leading-relaxed">{e.description}</p>
                )}
              </div>
            ))}
          </div>
      ...
      case 'skills':
        return resume.skills?.filter((s) => s.name)?.length ? (
          <div key={key} className="mb-4">
            ...
            <div data-keep-together className="flex flex-wrap gap-1.5">
        ...
        return resume.projects?.length ? (
          <div key={key} className="mb-4">
            ...
            {resume.projects.map((p, i) => (
              <div key={i} data-keep-together className={projectCardClass} style={projectCardStyle}>
                <div className="font-semibold text-xs text-gray-900">{p.name}</div>
                {p.url && <div><UrlLink url={p.url} className="text-[10px] underline" style={{ color: style.primary }} /></div>}
          ...
          <div key={key} className="mb-4">
            ...
            {resume.certifications.map((c, i) => (
              ...
              >
                ...
                {c.url && <div><UrlLink url={c.url} className="text-[10px] underline" style={{ color: style.primary }} /></div>}
              </div>
            ))}
          </div>
        ) : null;
  ...
  return (
    <div className="mb-4">
      ...
      <div className="relative pl-4 border-l-2 space-y-4" style={{ borderColor: `${style.primary}40` }}>
        {resume.experience.map((e, i) => (
          <div key={i} data-keep-together className="relative">
            <div
...
const LEVEL_WIDTHS = { beginner: 35, basic: 35, intermediate: 60, advanced: 80, expert: 95 };

// A recognised level sets the bar length; free-text levels keep the old
// decorative widths so the bars still vary.
const barWidth = (s, i) => LEVEL_WIDTHS[(s.level || '').trim().toLowerCase()] ?? 60 + (i % 4) * 10;

export const SkillBars = ({ skills, style, light }) => (
  <div className="space-y-2">
    {skills.filter((s) => s.name).map((s, i) => (
      <div key={i}>
        <div className={`text-[10px] mb-0.5 flex justify-between gap-2 ${light ? 'text-white/90' : 'text-gray-600'}`}>
          <span>{s.name}</span>
          {s.level && <span className="opacity-70 shrink-0">{s.level}</span>}
        </div>
